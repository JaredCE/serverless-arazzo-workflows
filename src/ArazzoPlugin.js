...
class ArazzoPlugin {
        ...
        commands: {
          generate: {
            ...
            options: {
              ...
              source: {
                usage:
                  "The default OpenAPI Source file to use [default: openapi.json]",
                shortcut: "s",
                type: "string",
              },
            },
          ...
          run: {
            ...
            options: {
              source: {
                usage:
                  "The default Arazzo Specification Source file to use [default: arazzo.json]",
                shortcut: "s",
                type: "string",
              },
              input: {
                usage:
                  "The input variables to fill in things like Query Strings and Path parameters [default: input.json]",
                shortcut: "i",
                type: "string",
              },
            },
  ...
  async generate() {
    ...
    await this.arazzoGeneration().catch((err) => {
      throw new this.serverless.classes.Error(err);
    });
  }

  async run() {
    this.logger.notice(chalk.bold.underline("Arazzo v1 Specification Runner"));
  ...
  processCLIInput() {
    ...
    if (
      this.serverless.processedInput?.options?.format?.toLowerCase() === "yaml"
    ) {
      this.serverless.processedInput.options.format = "yml";
    }

    config.format =
      this.serverless.processedInput?.options?.format?.toLowerCase() || "json";
