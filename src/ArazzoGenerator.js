...
class ArazzoGenerator {
  constructor(arazzoDocumentation, options) {
    ...
    try {
      ...
      this.REDOCLY_RULES = require(
        path.resolve("options", "redocly-arazzo.json"),
      );
    } catch (err) {
      this.REDOCLY_RULES = {
        struct: "error",
  ...
  generateSourceDescriptions() {
    ...
    if (this.arazzoDocumentation?.sourceDescriptions) {
      for (const sourceDescription of this.arazzoDocumentation
        ?.sourceDescriptions) {
  ...
  generateStepsFromEvents() {
            ...
            if (workflow.workflowName === this.currentWorkflow.workflowName) {
              if (
                Object.keys(workflow).some((key) =>
                  ["operationId", "workflowId", "operationPath"].includes(key),
                ) === false
              ) {
    ...
    return steps
      ...
      .map(({ stepNumber, ...keepAttrs }) => keepAttrs);
  }
  ...
  getViableFunctions() {
    const isViableFunction = (functionTypes) => {
      return Object.keys(functionTypes).some((functionType) =>
        this.viableArazzoKeys.includes(functionType),
      );
    };
