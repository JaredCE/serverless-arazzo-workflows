...
class Arazzo extends Document {
  ...
  async dealWithOutputs(response) {
    ...
    if (this.step?.outputs) {
      ...
      for (const key in this.step.outputs) {
        ...
        const isARuntimeValue = this.matchesExpectedRunTimeExpression(
          this.step.outputs[key],
          "$response.",
        );
  ...
  async dealWithSuccessfulResponse(response) {
    ...
    if (this.step.successCriteria) {
      successCriteriaPassed = this.dealWithCriteria(
        response,
  ...
  dealWithCriteria(response, criteriaArr) {
    ...
    for (const successCriteria of criteriaArr) {
      if (
        Object.hasOwn(successCriteria, "type") === false ||
        successCriteria?.type === "simple"
      ) {
  ...
  getOperationIdSourceDescription() {
    ...
    if (this.sourceDescriptions.length === 1) {
      ...
      if (
        this.matchesExpectedRunTimeExpression(
          operationOrWorkflowPointer,
          "$sourceDescriptions.",
        )
      ) {
        const sourceDescription = this.sourceDescriptions.filter(
          (sourceDescription) => {
            if (
              sourceDescription.name ===
              operationOrWorkflowPointer.split(".")[1]
            ) {
