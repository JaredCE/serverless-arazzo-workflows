  ...
  xdescribe(`runWorkflows`, function () {
    describe(`OpenAPI SourceDescriptions`, function () {
      ...
      xit(`should throw an error when a step fails onFailure at step level is included and is set to retry and all retries are exhausted`, async function () {
        ...
        const arazzo = new Arazzo(
          "./test/mocks/workingArazzoMockWithRetryOnFailure.json",
          "arazzo",
          { logger: logger, parser },
        );
        arazzo.setMainArazzo();
        ...
        try {
          ...
          expect(err.message).to.be.equal(
            `Call to GET http://petstore.swagger.io/v2/pet/findByStatus failed with a 404`,
          );
        }
    ...
    it(`should throw an error when there is more than one sourceDescription and it is incorrectly referenced`, async function () {
      nock("https://raw.githubusercontent.com:443", {
        ...
        .get(
          "/readmeio/oas-examples/refs/heads/main/3.1/json/petstore-simple.json",
        )
        .reply(404);
      ...
      const arazzo = new Arazzo(
        "./test/mocks/arazzoMockIncorrectlyReferencedSourceDescription.json",
        "arazzo",
        { logger: logger, parser },
      );
      arazzo.setMainArazzo();
      ...
      try {
        ...
        expect(err).to.be.instanceOf(Error);
        expect(err.message).to.be.equal(
          `No known matching source description for $sourceDescriptions.Arazzo-Workflow-for-simple-petstore-openAP.getById`,
        );
      }
    });
    ...
    it(`should throw an error when loading a sourceDescription from a URL fails`, async function () {
      ...
      try {
        ...
        expect(err).to.be.instanceOf(Error);
        expect(err.message).to.be.equal(
          `Error fetching document from https://raw.githubusercontent.com/readmeio/oas-examples/refs/heads/main/3.1/json/petstore.json`,
        );
      }
    });
